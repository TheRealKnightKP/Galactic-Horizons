...
function _buildStarBg() {
  ...
  for (let i = 0; i < 320; i++) { const x = (i * 367 + 11) % GAME_W; const y = (i * 211 + 53) % GAME_H; sc.fillRect(x, y, 1, 1); }
  for (let i = 0; i < 90; i++) {
    const x = (i * 491 + 137) % GAME_W; const y = (i * 313 + 79)  % GAME_H;
    const alpha = 0.45 + (i % 5) * 0.09;
    sc.fillStyle = "rgba(210,220,255," + alpha.toFixed(2) + ")";
    const sz = i % 3 === 0 ? 1.5 : 1; sc.fillRect(x, y, sz, sz);
  }
  for (let i = 0; i < 20; i++) {
    const x = (i * 719 + 233) % GAME_W; const y = (i * 503 + 171) % GAME_H;
    ...
    sc.fillStyle = glow; sc.beginPath(); sc.arc(x, y, 3, 0, Math.PI*2); sc.fill();
    sc.fillStyle = "#ffffff"; sc.fillRect(x, y, 1, 1);
  }
  ...
  sc.fillStyle = vig; sc.fillRect(0, 0, GAME_W, GAME_H);
}
...
function spawnThrusterParticles(obj, isPlayer, shipName) {
  ...
  const shape = isPlayer ? (typeof getThrusterShape==="function"?getThrusterShape(shipName):"classic") : "classic";
  const color = isPlayer ? (typeof getThrusterColor==="function"?getThrusterColor(shipName):THRUSTER_DEFAULT_COLOR) : "#8084ff";
  const cx = obj.x + obj.w/2, cy = obj.y + obj.h/2;
  const angle = obj.rotation || 0;
  const backX = cx - Math.cos(angle) * (obj.w * 0.5);
  const backY = cy - Math.sin(angle) * (obj.w * 0.5);
  const spd = Math.hypot(obj.vx||0, obj.vy||0);
  if (spd < 0.5 && !obj.boosting) return;
  const count = obj.boosting ? 4 : 2;
  for (let i = 0; i < count; i++) {
    if (_thrusterParticles.length >= MAX_THRUSTER_PARTICLES) break;
    let px=backX, py=backY, pvx, pvy, life, size;
    const base = spd * 0.3;
    switch(shape) {
      case "cone": pvx=-Math.cos(angle)*base+(Math.random()-0.5)*2.5; pvy=-Math.sin(angle)*base+(Math.random()-0.5)*2.5; life=14; size=4+Math.random()*3; break;
      case "ring": { const ra=Math.random()*Math.PI*2; px=backX+Math.cos(ra)*4; py=backY+Math.sin(ra)*4; pvx=-Math.cos(angle)*base*0.3; pvy=-Math.sin(angle)*base*0.3; life=12; size=3; break; }
      case "twin": { const side=(i%2===0?1:-1); px=backX+Math.cos(angle+Math.PI/2)*side*(obj.h*0.28); py=backY+Math.sin(angle+Math.PI/2)*side*(obj.h*0.28); pvx=-Math.cos(angle)*base*1.2; pvy=-Math.sin(angle)*base*1.2; life=13; size=3; break; }
      case "spiral": { const sa=angle+Math.PI+(frameCount*0.3+i*Math.PI); pvx=Math.cos(sa)*base; pvy=Math.sin(sa)*base; life=16; size=2.5; break; }
      case "burst": { const ba=Math.random()*Math.PI*2; pvx=Math.cos(ba)*(base*0.8+Math.random()*1.5); pvy=Math.sin(ba)*(base*0.8+Math.random()*1.5); life=10; size=2; break; }
      case "blade": pvx=-Math.cos(angle)*base*1.5+(Math.random()-0.5)*0.5; pvy=-Math.sin(angle)*base*1.5+(Math.random()-0.5)*0.5; life=8; size=1.5+Math.random()*4; break;
      case "ghost": pvx=-Math.cos(angle)*base*0.4+(Math.random()-0.5)*1; pvy=-Math.sin(angle)*base*0.4+(Math.random()-0.5)*1; life=22; size=5+Math.random()*4; break;
      default: pvx=-Math.cos(angle)*base+(Math.random()-0.5)*1.2; pvy=-Math.sin(angle)*base+(Math.random()-0.5)*1.2; life=12; size=3+Math.random()*2;
    }
    if (obj.boosting) { life=Math.round(life*1.5); size*=1.3; }
    _thrusterParticles.push({ x:px, y:py, vx:pvx, vy:pvy, life, maxLife:life, color, shape, size });
  }
}

function updateThrusterParticles() {
  _thrusterParticles.forEach(p => { p.x+=p.vx; p.y+=p.vy; p.vx*=0.88; p.vy*=0.88; p.life--; });
  _thrusterParticles = _thrusterParticles.filter(p => p.life > 0);
}

function drawThrusterParticles() {
  if (_thrusterParticles.length === 0) return;
  const byColor = {};
  for (const p of _thrusterParticles) { (byColor[p.color] = byColor[p.color] || []).push(p); }
  for (const [color, particles] of Object.entries(byColor)) {
    ...
    for (const p of particles) {
      ...
      if (p.shape === "blade") { const len=p.size*t; ctx.lineWidth=2*t; ctx.beginPath(); ctx.moveTo(p.x,p.y); ctx.lineTo(p.x+p.vx*len,p.y+p.vy*len); ctx.stroke(); }
      else if (p.shape === "ring") { ctx.lineWidth=1.5; ctx.beginPath(); ctx.arc(p.x,p.y,p.size*(2-t),0,Math.PI*2); ctx.stroke(); }
      else { const r=Math.max(0.5,p.size*t); ctx.beginPath(); ctx.arc(p.x,p.y,r,0,Math.PI*2); ctx.fill(); }
    }
    ctx.restore();
  }
}
...
let frameCount = 0;
let playerDodgeHistory = [];
let _prevPlayerVx = 0, _prevPlayerVy = 0;
let allyFormation = "behind";
let isDeployed        = false;
let _gKeyPrev         = false;
let capitalShipObj    = null;
let deployedShipKey   = null;
let deployedShipAvail = true;
let capitalNoRespawn  = 0;
let capitalDestroyed  = false;
let playerTookDamageThisWave = false;
let _waveMissileFired = false;
let _thrusterParticles = [];
let _enemyCapFormation = "default";
let _enemyCapTimer     = 0;
let _enemyCapRegen     = 1.0;
let _playerYHistory    = [];
...
function updatePlayerDodgeTracking() {
  ...
  _prevPlayerVx = vx; _prevPlayerVy = vy;
}
...
function resolveEnemyShotFeedback(e) {
  ...
  e.shotFeedback = e.shotFeedback.filter(fb => {
    if(frameCount - fb.frame < 25) return true;
    const actualX = player.x + player.w/2, actualY = player.y + player.h/2;
    const errX = actualX - fb.predictedX, errY = actualY - fb.predictedY;
    const dodge = player.dodgeBase || 0;
    const noiseX = (Math.random()-0.5) * dodge * 40, noiseY = (Math.random()-0.5) * dodge * 40;
  ...
  if(e.leadCorrection) {
    ...
    e.leadCorrection.x *= decay; e.leadCorrection.y *= decay;
  }
}
...
function getAdaptiveAimPos(e, rawPredX, rawPredY) {
  ...
  e.leadCorrection = e.leadCorrection || {x:0, y:0};
  let ax = rawPredX + e.leadCorrection.x, ay = rawPredY + e.leadCorrection.y;
  if(playerDodgeHistory.length >= 3) {
    const dvx = (player.vx||0) - _prevPlayerVx, dvy = (player.vy||0) - _prevPlayerVy;
    ...
    if(isMidDodge) {
      ...
      for(let i = playerDodgeHistory.length - recentCount; i < playerDodgeHistory.length; i++) {
        avgPostVx += playerDodgeHistory[i].postVx; avgPostVy += playerDodgeHistory[i].postVy;
      }
      avgPostVx /= recentCount; avgPostVy /= recentCount;
      const dodge = player ? (player.dodgeBase || 0) : 0;
      const dodgeHistoryWeight = (1.0 - dodge * 0.7) * wavScale * 0.4;
      ax += avgPostVx * 18 * dodgeHistoryWeight; ay += avgPostVy * 18 * dodgeHistoryWeight;
    }
  }
  return {x: ax, y: ay};
}
...
function adaptivePredictAndRecord(e, targetX, targetY, tvx, tvy, fromX, fromY, speed) {
  ...
  if(e.shotFeedback.length < 20) e.shotFeedback.push({frame: frameCount, predictedX: adj.x, predictedY: adj.y});
  return adj;
}
...
function getHitFace(bullet, target) {
  ...
  const impactAngle = Math.atan2((bullet.y + (bullet.h||0)/2) - tcy, (bullet.x + (bullet.w||0)/2) - tcx);
...
function buildAlly(i,totalSlots) {
  ...
  if(!slot||!slot.ship||slot._occupied) return null;
  ...
  let wStats=null;
  if(wStatBase){
    wStats={...wStatBase, damage:Math.round(wStatBase.damage*beh.dmgMult*wqMult2), fireInterval:Math.round(wStatBase.fireInterval/beh.rpmMult)};
  }
...
function setPlayerShip(name) {
  if (typeof isDeployed !== "undefined" && isDeployed && typeof capitalShipObj !== "undefined" && capitalShipObj) {
    capitalShipObj._isCapitalAutopilot = false; capitalShipObj = null; isDeployed = false;
  }
  ...
  const _mb = (typeof getMasteryBonus === "function") ? getMasteryBonus(name) : null;
  const _mbHp=_mb?.maxHpMult||1.0, _mbShield=_mb?.maxShieldsMult||1.0, _mbSpeed=_mb?.speedMult||1.0;
  const _mbDodge=_mb?.dodgeBonus||0, _mbDmg=_mb?.weaponDamageMult||1.0, _mbFR=_mb?.fireRateMult||1.0;
  const _mastHp=Math.round(baseHp*_mbHp), _mastSh=Math.round(baseShields*_mbShield), _mastSpd=baseSpeed*_mbSpeed;
  const _mastDodge=Math.min(0.90, dodgeBase+_mbDodge), _mastDodgeB=Math.min(0.95, dodgeBoosted+_mbDodge);
  const _mastWStats=wStats?{...wStats, damage:Math.round(wStats.damage*_mbDmg), fireInterval:Math.max(1, Math.round(wStats.fireInterval/_mbFR))}:null;

  player={
    x:80, y:GAME_H/2-20, w:Math.round((40+(d.size||1)*16)*SIZE_SCALE), h:Math.round((24+(d.size||1)*8)*SIZE_SCALE),
    hp:_mastHp, maxHp:_mastHp, shields:_mastSh, maxShields:_mastSh,
    armor:baseArmor, maxArmor:baseArmor, armorType:d.armorType||"light",
    missiles:(playerLoadout.missileRack||[]).length, maxMissiles:(playerLoadout.missileRack||[]).length,
    speed:_mastSpd, maxSpeed:_mastSpd, accel:_mastSpd*0.20,
    weaponType:wType, weaponSize:d.weaponSize, weaponStats:_mastWStats,
...
function spawnShadowCometWave() {
  ...
  const sc = {
    type: "ShadowComet",
    x: GAME_W * 0.7, y: GAME_H / 2 - eh / 2, w: ew, h: eh,
    ...
    isShadowComet: true,
    animFrames: d.animFrames || null,
    animPattern: d.animPattern || null,
    animSpeed: d.animSpeed || 20,
    damageMult: scaled.damage || 1,
    dodgeTimer: 0, repositionTimer: 0, repositionTarget: null,
    rotation: Math.PI, turnSpeed: 0.12, turrets: [],
  };
  initShieldFaces(sc);
  window._pendingShadowComet = sc;
  player.x = GAME_W * 0.25; player.y = GAME_H / 2 - player.h / 2;
  player.vx = 0; player.vy = 0;
  const _scx = GAME_W * 0.7 + 20, _scy = GAME_H / 2;
...
function updateShadowCometCutscene() {
  ...
  if (shadowCometCutsceneState === "intercepted" && shadowCometCutsceneTimer <= 0) {
    shadowCometCutsceneState = "dialogue"; shadowCometCutsceneTimer = 150;
  } else if (shadowCometCutsceneState === "dialogue" && shadowCometCutsceneTimer <= 0) {
    shadowCometCutsceneState = "fighting";
    if (window._pendingShadowComet) { enemies.push(window._pendingShadowComet); window._pendingShadowComet = null; }
    state = "playing";
  }
}
...
function drawShadowCometCutscene() {
  ...
  if (shadowCometCutsceneState === "intercepted") {
    ...
    ctx.fillStyle = "#eee"; ctx.font = "15px monospace"; ctx.fillText("\"It's just you and me!\"", GAME_W/2, GAME_H/2 - 4); ctx.restore();
    if (window._pendingShadowComet) {
      const sc = window._pendingShadowComet;
      ctx.save(); ctx.fillStyle = "rgba(0,0,0,0.7)"; ctx.fillRect(sc.x + sc.w/2 - 80, sc.y - 34, 160, 22);
      ctx.fillStyle = "#ff4444"; ctx.font = "bold 13px monospace"; ctx.fillText("Shadow Comet", sc.x + sc.w/2, sc.y - 18); ctx.restore();
...
function updateShadowCometAI(e) {
  ...
  if (e.dodgeTimer <= 0) {
    ...
    e.vx += Math.cos(perpAngle) * e.speed * 1.2; e.vy += Math.sin(perpAngle) * e.speed * 1.2;
  }
  e.vx *= 0.88; e.vy *= 0.88;
  const spd = Math.hypot(e.vx, e.vy);
  if (spd > e.speed * 2) { e.vx *= (e.speed*2)/spd; e.vy *= (e.speed*2)/spd; }
  e.x += e.vx; e.y += e.vy;
  e.x = Math.max(10, Math.min(GAME_W - e.w - 10, e.x)); e.y = Math.max(10, Math.min(GAME_H - e.h - 10, e.y));
  e.shootTimer--;
  if (e.shootTimer <= 0) {
...
function isCometFullyUpgraded() {
  const ups = shipUpgrades["Comet"]; if (!ups) return false;
...
function fullUpgradeComet() {
  ...
  if (currentShipName === "Comet") { loadShipUpgrades("Comet"); setPlayerShip("Comet"); }
}
...
function spawnShadowVenganceWave() {
  ...
  const sv = {
    ...
    x: GAME_W * 0.68, y: GAME_H / 2 - eh / 2, w: ew, h: eh,
    hp: d.hp, maxHp: d.hp, shields: d.shields, maxShields: d.shields,
    armor: d.armor, maxArmor: d.armor, armorType: d.armorType,
    speed: d.speed, fireRate: d.fireRate, shootTimer: 30,
    img: getImage(typeof SHADOW_VENGEANCE_ANIM !== "undefined" ? SHADOW_VENGEANCE_ANIM.image : d.image), color: d.color, score: 0,
    spriteAngleOffset: Math.PI,
    stunTimer: 0, vx: 0, vy: 0,
    isShadowVengance: true,
    animFrames: typeof SHADOW_VENGEANCE_ANIM !== "undefined" ? SHADOW_VENGEANCE_ANIM.animFrames : null,
    animPattern: typeof SHADOW_VENGEANCE_ANIM !== "undefined" ? SHADOW_VENGEANCE_ANIM.animPattern : null,
    animSpeed: typeof SHADOW_VENGEANCE_ANIM !== "undefined" ? SHADOW_VENGEANCE_ANIM.animSpeed : 20,
    dodgeTimer: 0, repositionTimer: 0, repositionTarget: null,
    rotation: Math.PI, turnSpeed: 0.14, turrets: [],
    revengeActive: false, revengeTimer: 0,
    burstState: 0, burstTimer: 0,
  };
...
function updateShadowVenganceCutscene() {
  ...
  if (shadowVenganceCutsceneState === "intercepted" && shadowVenganceCutsceneTimer <= 0) {
    shadowVenganceCutsceneState = "dialogue"; shadowVenganceCutsceneTimer = 200;
  } else if (shadowVenganceCutsceneState === "dialogue" && shadowVenganceCutsceneTimer <= 0) {
    shadowVenganceCutsceneState = "fighting";
    if (window._pendingShadowVengance) { enemies.push(window._pendingShadowVengance); window._pendingShadowVengance = null; }
    state = "playing";
  }
}
...
function drawShadowVenganceCutscene() {
  ...
  if (shadowVenganceCutsceneState === "intercepted") {
    ...
    ctx.fillStyle = "#eee"; ctx.font = "15px monospace"; ctx.fillText("You thought this was over?! Im not done until I kill you!", GAME_W/2, GAME_H/2 - 8); ctx.restore();
    if (window._pendingShadowVengance) {
      const sv = window._pendingShadowVengance;
      ctx.save(); ctx.fillStyle = "rgba(0,0,0,0.7)"; ctx.fillRect(sv.x + sv.w/2 - 90, sv.y - 34, 180, 22);
      ctx.fillStyle = "#cc0033"; ctx.font = "bold 13px monospace"; ctx.fillText("Shadow Vengeance", sv.x + sv.w/2, sv.y - 18); ctx.restore();
...
function updateShadowVenganceAI(e) {
  ...
  if (e.repositionTimer <= 0) {
    ...
    e.repositionTarget = { x: GAME_W * (0.50 + Math.random() * 0.40), y: 50 + Math.random() * (GAME_H - 100) };
  }
  if (e.repositionTarget) {
    const tdx = e.repositionTarget.x - e.x, tdy = e.repositionTarget.y - e.y, dist = Math.hypot(tdx, tdy) || 1;
    e.vx += (tdx/dist) * e.speed * 0.4; e.vy += (tdy/dist) * e.speed * 0.4;
  }
  e.dodgeTimer = (e.dodgeTimer||0) - 1;
  if (e.dodgeTimer <= 0) {
    e.dodgeTimer = 14 + Math.floor(Math.random() * 24);
    const perpAngle = e.rotation + (Math.random() > 0.5 ? Math.PI/2 : -Math.PI/2);
    e.vx += Math.cos(perpAngle) * e.speed * 1.4; e.vy += Math.sin(perpAngle) * e.speed * 1.4;
  }
  if (!e.revengeActive && e.hp < e.maxHp * 0.5) { e.revengeActive = true; e.speed *= 1.4; e.dodgeChance = 0.62; }
  e.vx *= 0.87; e.vy *= 0.87;
  const spd = Math.hypot(e.vx, e.vy);
  ...
  e.burstTimer--;
  if (e.burstTimer <= 0) {
    if (e.burstState === 0) { _svFire(e, ecx, ecy, pcx, pcy); e.burstState = 1; e.burstTimer = 8; }
    else if (e.burstState === 1) { _svFire(e, ecx, ecy, pcx, pcy); e.burstState = 2; e.burstTimer = e.fireRate - 8; }
    else { e.burstState = 0; e.burstTimer = 0; }
  }
}
...
function isVenganceFullyUpgraded() {
  const ups = shipUpgrades["Vengeance"]; if (!ups) return false;
  return ups.shieldTier >= 3 && ups.armorTier >= 3 && ups.engineTier >= 3;
}
...
function fullUpgradeVengance() {
  ...
  if (currentShipName === "Vengeance") { loadShipUpgrades("Vengeance"); setPlayerShip("Vengeance"); }
}
...
function createEnemyObject(name, spawnX, spawnY) {
  ...
  const e = {
    ...
    spriteAngleOffset: Math.PI, stunTimer: 0, vx: 0, vy: 0,
  ...
  // Copy animation data if defined
  if (d.animFrames) {
    e.animFrames = d.animFrames;
    e.animPattern = d.animPattern;
    e.animSpeed = d.animSpeed || 20;
  }
  initShieldFaces(e);
  return e;
}
...
function spawnWave() {
  ...
  if (waveData && waveData.shadowCometWave) {
    ...
    if(IS_MOBILE){const ui=document.getElementById("mobileUI");if(ui)ui.style.display="block";} return;
  }
  if (waveData && waveData.shadowVenganceWave) {
    spawnShadowVenganceWave(); state = "shadowVenganceCutscene";
    document.getElementById("hud").style.display = "block"; document.getElementById("inGameBack").style.display = "block";
    if(IS_MOBILE){const ui=document.getElementById("mobileUI");if(ui)ui.style.display="block";} return;
  }
  const wrd=waveData.reinforceDelay||0; if(wrd>0) waveReinforceTimer=wrd;
  waveData.enemies.forEach(name=>{
    const d=ENEMIES[name];if(!d)return;
    const sizeNum=d.size||2; const ew=Math.round((32+sizeNum*18)*SIZE_SCALE);
...
function nextWave() {
  ...
  pdcDisabledThisWave = false; shadowCometActive = false; shadowVenganceActive = false; shadowCometTurretLimit = false;
  if (isDeployed && capitalShipObj) recallToCapital();
  isDeployed = false; capitalShipObj = null; deployedShipAvail = true; capitalDestroyed = false;
  if (capitalNoRespawn > 0) capitalNoRespawn--;
  respawnDeadAllies(); spawnWave();
  if (state !== "shadowCometCutscene" && state !== "shadowVenganceCutscene") {
    state="playing"; document.getElementById("hud").style.display="block"; document.getElementById("inGameBack").style.display="block";
...
function applyDamage(target,bullet) {
  ...
  if(target.isAlly){
    ...
    const allyDodge=target.dodge||0.25; if(Math.random()<allyDodge)return;
    let dmgMult=0.75;
  ...
  if(bullet.missile){
    ...
    else target.hp-=bullet.damage; return;
  }
  ...
  if (bullet.critChance && Math.random() < bullet.critChance) {
    critBoost = bullet.critMult || 3.0;
    if (target !== player && !target.isAlly) spawnHitEffect(target.x+target.w/2, target.y+target.h/2, {...bullet, category:"railgun", size: wSize+2});
    if (typeof recordCareerStat === "function") recordCareerStat("railgunCrits");
  }
  let shieldDmg, hullDmg;
  if (cat==="ballistic") { shieldDmg = rawDmg * 0.25; hullDmg = rawDmg * 0.5 * stunSynergy * dirMult * critBoost; }
  else if (cat==="lc_ballistic") { const shieldsUp = (target.shields||0) > 0; shieldDmg = rawDmg * (shieldsUp ? 0.50 : 0); hullDmg = rawDmg * (shieldsUp ? 0.50 : 0.90) * dirMult * critBoost; }
  else if (cat==="distortion") { shieldDmg = rawDmg * 0.15; hullDmg = rawDmg * 0.10; }
  else if (cat==="phase") { shieldDmg = 0; hullDmg = rawDmg * 0.5 * dirMult * critBoost; }
  else if (cat==="corrosion") { shieldDmg = rawDmg * 0.5; hullDmg = rawDmg * 0.3 * dirMult;
    if (!target.corrosionTimer) { target.corrosionTimer = 180; target.corrosionDPS = (bullet.corrosionDPS || 0.18) * rawDmg; }
    else target.corrosionTimer = Math.max(target.corrosionTimer, 180);
  ...
  const hullExposed = cat==="phase" || cat==="lc_ballistic" || faceWasDown || faceIsDownNow || cat==="ballistic" || cat==="distortion";
  if (hullExposed) {
  ...
  if (bullet.staggerOnHit && !target.staggerTimer) {
    ...
    if (typeof personalRecords !== "undefined") personalRecords.staggerKills = (personalRecords.staggerKills||0) + 0.01;
  }
  if (cat==="distortion") {
  ...
  if (bullet.gravityPull && !target.gravityPullTimer) {
    target.gravityPullTimer = bullet.gravityPullDur || 90;
    target.gravityPullX = bullet.x + (bullet.vx||0)*2; target.gravityPullY = bullet.y + (bullet.vy||0)*2;
  }
}
...
function fireBullets(origin,wStats,angle,isPlayer) {
  ...
  const base={category:wStats.category,weaponSize:wStats.size,penetration:wStats.penetration,
              ...
              shadowPhaseChance:wStats.shadowPhaseChance, shadowTrail:wStats.shadowTrail,
              gravityPull:wStats.gravityPull, gravityPullDur:wStats.gravityPullDur,
              vortexPull:wStats.vortexPull, vortexPullDur:wStats.vortexPullDur,
              shieldBypassChance:wStats.shieldBypassChance, corrosionDPS:wStats.corrosionDPS };
  ...
  const dmgScale=(!isPlayer&&(wStats.category==="ballistic"||wStats.category==="lc_ballistic"))?0.5:1.0;
  const inaccuracy=isPlayer?0:getEnemyInaccuracySpread();
  if (isPlayer && typeof recordShotFired === "function") recordShotFired(wStats.category);
  if(wStats.hitscan){
    if(!isPlayer){ const cos=Math.cos(angle),sin=Math.sin(angle); const ep=rayEndpoint(bx,by,cos,sin); beamFlashes.push({x1:bx,y1:by,x2:ep.x,y2:ep.y,life:12,maxLife:12,color:col||"#ffff88"}); }
    return[];
  }
  if(isPlayer)playShootSound(wStats.category,true);
  if (wStats.novaFan) {
    const shots = wStats.fanShots || 3;
    return Array.from({length:shots}, (_,i) => {
      const spread = wStats.spread || 0.07;
      const a = angle + (i - Math.floor(shots/2)) * spread + (Math.random()-0.5)*0.02;
      return {...base, x:bx, y:by, vx:Math.cos(a)*scaledSpeed, vy:Math.sin(a)*scaledSpeed, w:wStats.w, h:wStats.h, damage:wStats.damage*dmgScale, maxRange:GAME_W*0.9, distTraveled:0};
    });
  }
  if (wStats.voidSelfDmg && isPlayer) {
    if (player.shields > 0) player.shields = 0;
    if (capitalShipObj && !capitalDestroyed) capitalShipObj.shields = Math.max(0, (capitalShipObj.shields||0) - wStats.damage*0.5);
  }
  ...
  const bullet = {...base,x:bx,y:by,vx:Math.cos(a)*scaledSpeed,vy:Math.sin(a)*scaledSpeed,w:wStats.w,h:wStats.h,damage:wStats.damage*dmgScale};
  if (wStats.aoeRadius) { bullet.aoeRadius = wStats.aoeRadius; bullet.voidSelfDmg = wStats.voidSelfDmg; }
  return [bullet];
}
...
function drawRailgunCharge() {
  ...
  const bx=cx+Math.cos(player.rotation)*frontDist, by=cy+Math.sin(player.rotation)*frontDist;
  const maxRadius=18+(player.weaponSize||1)*3, r=maxRadius*chargeT;
  ...
  coreGrad.addColorStop(0,"#ffffff"); coreGrad.addColorStop(0.3,"#aaddff"); coreGrad.addColorStop(1,"#0066ff");
  ctx.fillStyle=coreGrad; ctx.shadowColor="#00aaff"; ctx.shadowBlur=20*chargeT;
  ...
  if(chargeT>0.6){
    ...
    for(let i=0;i<sparkCount;i++){
      ...
      ctx.beginPath(); ctx.moveTo(bx+Math.cos(sa)*r,by+Math.sin(sa)*r); ctx.lineTo(bx+Math.cos(sa)*sr,by+Math.sin(sa)*sr); ctx.stroke();
    }
  }
  ctx.restore();
}
...
function showSpecialToast(msg) {
  ...
  if(!t){ t=document.createElement("div");t.id="specialToast"; t.style.cssText="position:fixed;top:90px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.88);color:#ff8800;font:bold 15px monospace;padding:5px 18px;border:1px solid #ff8800;border-radius:4px;z-index:9999;pointer-events:none;transition:opacity 0.4s"; document.body.appendChild(t); }
  t.textContent=msg;t.style.opacity="1"; clearTimeout(t._t);t._t=setTimeout(()=>t.style.opacity="0",1800);
}

function spawnWaspAlly() {
  let bestShip="Sprite",bestPrice=-1;
  for(const sn of ALLY_SHIP_ORDER){ const av=(typeof allyInventory!=="undefined"?allyInventory[sn]||0:0)-(playerLoadout.allies.filter(a=>a&&a.ship===sn).length); const pr=ALLY_SHIP_DEFS[sn]?.price||0; if(av>0&&pr>=bestPrice){bestPrice=pr;bestShip=sn;} }
  const aDef=ALLY_SHIP_DEFS[bestShip]||ALLY_SHIP_DEFS.Sprite;
  const wStats=getWeaponStats("laser_repeater",aDef.weaponSize);
  allies.push({ type:bestShip, name:"[TEMP] "+bestShip, x:player.x-60,y:player.y, w:Math.round(aDef.w*SIZE_SCALE),h:Math.round(aDef.h*SIZE_SCALE), hp:aDef.hp,maxHp:aDef.hp,shields:aDef.shields,maxShields:aDef.shields, armor:100,maxArmor:100,armorType:aDef.armorType, img:getImage(aDef.image),color:"#ffdd88", shootTimer:10,vx:0,vy:0,rotation:0,spriteAngleOffset:Math.PI, weaponType:"laser_repeater",weaponSize:aDef.weaponSize,weaponStats:wStats, isAlly:true,dodge:0.25,speedMult:1.0, tempAlly:true,tempTimer:600 });
}
...
function spawnRetributionAllies() {
  ...
  allies.push({ type:"Comet", name:"[RETRIB] Comet", x:player.x-60, y:player.y-50, w:Math.round(cometDef.w*SIZE_SCALE), h:Math.round(cometDef.h*SIZE_SCALE), hp:cometDef.hp*3, maxHp:cometDef.hp*3, shields:cometDef.shields*3, maxShields:cometDef.shields*3, armor:100, maxArmor:100, armorType:"light", img:getImage(cometDef.image), color:"#ff2200", shootTimer:5, vx:0, vy:0, rotation:0, spriteAngleOffset:Math.PI, weaponType:"ballistic_cannon", weaponSize:5, weaponStats:cWStats, isAlly:true, dodge:0.55, speedMult:1.5, retributionAlly:true, tempTimer:600 });
  allies.push({ type:"Vengeance", name:"[RETRIB] Vengeance", x:player.x-60, y:player.y+50, w:Math.round(vengDef.w*SIZE_SCALE), h:Math.round(vengDef.h*SIZE_SCALE), hp:vengDef.hp*3, maxHp:vengDef.hp*3, shields:vengDef.shields*3, maxShields:vengDef.shields*3, armor:100, maxArmor:100, armorType:"medium", img:getImage(vengDef.image), color:"#ff0044", shootTimer:8, vx:0, vy:0, rotation:0, spriteAngleOffset:Math.PI, weaponType:"vengeance_cannon", weaponSize:8, weaponStats:vWStats, isAlly:true, dodge:0.45, speedMult:1.3, retributionAlly:true, tempTimer:600 });
...
function updateSpecial() {
  ...
  player.specialTimer--;
  switch(currentShipName){
    case"Tempest":{ const heal=player.maxHp*0.0015; player.hp=Math.min(player.maxHp,player.hp+heal); allies.forEach(a=>{if(!a.tempAlly)a.hp=Math.min(a.maxHp,a.hp+a.maxHp*0.0015);}); break;}
    case"Prometheus":{
      ...
      if(player.specialSalvoTimer<=0&&player.specialSalvoCount<player.specialSalvoTotal){
        ...
        const mKind=_rackEntry.kind||"standard", mTier=_rackEntry.tier||player.missileType||2;
        ...
        const mt=MISSILE_TYPES[mTier]||MISSILE_TYPES[2]; const baseDmg=mt.damage*(mkDef.damageMult||1.0);
        let lockTarget=null;
        if(mkDef.lockOn&&enemies.length>0){ const pcx2=player.x+player.w/2,pcy2=player.y+player.h/2; const cos2=Math.cos(player.rotation),sin2=Math.sin(player.rotation); let bestScore=1e9; enemies.forEach(en=>{ const ex=en.x+en.w/2,ey=en.y+en.h/2; const dt=(ex-pcx2)*cos2+(ey-pcy2)*sin2; if(dt<0)return; const perp=Math.abs((ey-pcy2)*cos2-(ex-pcx2)*sin2); if(perp<bestScore){bestScore=perp;lockTarget=en;} }); }
        ...
        if(mKind==="cluster"){ for(let ci=0;ci<4;ci++){ const a=aim+(ci-1.5)*0.18; playerBullets.push({x:player.x+player.w/2,y:player.y+player.h/2,vx:Math.cos(a)*mt.speed*1.2,vy:Math.sin(a)*mt.speed*1.2,w:10,h:5,damage:baseDmg*0.3,color:"#ffaa00",missile:true,category:"missile",weaponSize:4,lockTarget:lockTarget||null}); } }
        else { const aoe=mkDef.aoe||0; const mColor=mKind==="emp"?"#44ffcc":mKind==="nuke"?"#ff2200":mKind==="micro"?"#ffcc44":mt.color; playerBullets.push({x:player.x+player.w/2,y:player.y+player.h/2,vx:Math.cos(aim)*mt.speed,vy:Math.sin(aim)*mt.speed,w:mKind==="nuke"?28:mKind==="micro"?10:18,h:mKind==="nuke"?16:mKind==="micro"?5:8,damage:baseDmg,color:mColor,missile:true,category:"missile",weaponSize:mKind==="nuke"?8:6,missileKind:mKind,aoeRadius:aoe,corrosion:mkDef.corrosion,friendlyFire:mkDef.friendly,lockTarget:lockTarget||null}); }
        player.specialSalvoCount++; player.specialSalvoTimer=Math.ceil(sp.duration/Math.max(1,player.specialSalvoTotal));
      }
      break;}
  }
  if(player.specialTimer<=0){
    if(currentShipName==="Supernova"&&player.specialMissilesUsed===0){
      const fullRack=playerLoadout.missileRack||[]; const refillCount=Math.max(1,Math.ceil(fullRack.length*0.15)); const currentLen=player.missileRack.length;
      if(fullRack.length>0&&currentLen<fullRack.length){ const toAdd=fullRack.slice(currentLen,currentLen+refillCount); player.missileRack.push(...toAdd); player.missiles=player.missileRack.length; }
    }
    if(currentShipName==="Leviathan") allies.forEach(a=>{a.vanguardActive=false;a.vanguardRPM=false;});
    if(currentShipName==="Retribution"){ allies=allies.filter(a=>!a.retributionAlly); player.retributionSpeedBuff = false; }
    player.specialActive=false; player.specialCooldown=sp.cooldown;
  }
}
...
// ============================================================
function isCurrentShipCapital() { return typeof CAPITAL_SHIPS !== "undefined" && CAPITAL_SHIPS.has(currentShipName); }

function getDeployableShips() {
  if (!isCurrentShipCapital()) return [];
  const limits = typeof CAPITAL_DEPLOY_LIMITS !== "undefined" ? CAPITAL_DEPLOY_LIMITS[currentShipName] : null; if (!limits) return [];
  const maxSize = limits.maxSize || 1, onlyComet = limits.onlyComet || false;
  const result = [], allShips = typeof SHIPS !== "undefined" ? SHIPS : {};
  const effectiveOwned = [...ownedShips]; if (!effectiveOwned.includes("Starlight")) effectiveOwned.push("Starlight");
  for (const key of Object.keys(allShips)) {
    if (!effectiveOwned.includes(key)) continue; const sz = allShips[key].size || 1;
    if (onlyComet && key !== "Comet") continue; if (!onlyComet && sz > maxSize) continue;
    if (typeof CAPITAL_SHIPS !== "undefined" && CAPITAL_SHIPS.has(key)) continue; result.push(key);
  }
  return result;
}
...
function deployFromCapital() {
  ...
  if (!deployKey || (!ownedShips.includes(deployKey) && deployKey !== playerLoadout.ship)) { deployKey = validShips[0] || null; if (!deployKey) { const lim = typeof CAPITAL_DEPLOY_LIMITS !== "undefined" ? CAPITAL_DEPLOY_LIMITS[currentShipName] : null; if (lim) deployKey = "Starlight"; } }
  if (!deployKey) return;
  const deployDef = SHIPS[deployKey]; if (!deployDef) return;
  capitalShipObj = player; capitalShipObj._isCapitalAutopilot = true; capitalShipObj._capitalDriftDir = 1; capitalShipObj._capitalDriftTimer = 0; capitalShipObj._capitalTurretNerfMult = 1.25; capitalShipObj._deployGraceCleared = false;
  const spriteOffset = Math.PI, engTier = ENGINE_UPGRADE_TIERS[playerLoadout.engineTier||1], sn = deployKey, dDef = SHIPS[sn];
  const shieldMult = SHIELD_TIERS[playerLoadout.shieldTier||1].mult, armorMult = ARMOR_UPGRADE_TIERS[playerLoadout.armorTier||1].mult;
  const sizeNum = dDef.size||1, dw = Math.round((40+sizeNum*16)*SIZE_SCALE), dh = Math.round((24+sizeNum*8)*SIZE_SCALE);
  const deployedObj = {
    x: capitalShipObj.x + capitalShipObj.w/2 - dw/2, y: capitalShipObj.y + capitalShipObj.h/2 - dh/2, w: dw, h: dh,
    ...
    armor: Math.round(dDef.armor*armorMult), maxArmor: Math.round(dDef.armor*armorMult), armorType: dDef.armorType||"light",
    speed: dDef.speed*engTier.speedMult, maxSpeed: dDef.speed*engTier.speedMult, accel: dDef.speed*engTier.speedMult*0.20,
    ...
    missileRack: [...(capitalShipObj.missileRack||[])], missileActiveKind: capitalShipObj.missileActiveKind, missileType: dDef.missileType||2,
    img: getImage(dDef.image), color: dDef.color, rotation: 0, spriteAngleOffset: spriteOffset,
    vx: 0, vy: 0, shootTimer: 0, boosting: false, boostTimer: 0, boostCooldown: 0,
    ...
    dodgeBase: 0.09, dodgeBoosted: 0.18, specialCooldown: 0, specialActive: false, specialTimer: 0,
    railgunCharge: 0, railgunCharging: false, turrets: [], shipName: sn, _isDeployedShip: true,
  };
...
function updateCapitalAutopilot() {
  ...
  cap.vx = (cap.vx||0) * 0.92; cap.x = Math.max(0, Math.min(GAME_W*0.35, cap.x + cap.vx));
  if (enemies.length > 0) {
    const ccx=cap.x+cap.w/2, ccy=cap.y+cap.h/2; let closest=null, cd=1e9;
    enemies.forEach(e=>{const d=Math.hypot(e.x+e.w/2-ccx,e.y+e.h/2-ccy);if(d<cd){cd=d;closest=e;}});
    if (closest) { const targetRot = Math.atan2(closest.y+closest.h/2-ccy, closest.x+closest.w/2-ccx); let rd = targetRot - (cap.rotation||0); while(rd>Math.PI)rd-=Math.PI*2; while(rd<-Math.PI)rd+=Math.PI*2; cap.rotation = (cap.rotation||0) + Math.sign(rd)*Math.min(Math.abs(rd), cap.turnSpeed||0.015); }
  }
  cap.turrets && cap.turrets.forEach(t => {
    t.shootTimer--; if (t.shootTimer <= 0 && t.weaponStats) {
      const tx=cap.x+t.rx, ty=cap.y+t.ry;
      const nearestE = enemies.reduce((best,e)=>{ const d=Math.hypot(e.x+e.w/2-tx,e.y+e.h/2-ty); return (!best||d<best.d)?{e,d}:best; }, null);
      if (nearestE) { const pred = predictPos(nearestE.e.x+nearestE.e.w/2, nearestE.e.y+nearestE.e.h/2, nearestE.e.vx||0, nearestE.e.vy||0, tx, ty, t.weaponStats.speed||8); const angle = Math.atan2(pred.y-ty, pred.x-tx); playerBullets.push(...fireBullets({x:tx-1,y:ty-1,w:2,h:2}, t.weaponStats, angle, true)); }
      t.shootTimer = Math.round(t.fireRate * 1.25);
    }
  });
  if (cap.weaponStats && cap.weaponType && cap.weaponType !== "none") {
    ...
    if (cap.shootTimer <= 0 && enemies.length > 0) {
      ...
      if (target) { const pred = predictPos(target.x+target.w/2, target.y+target.h/2, target.vx||0, target.vy||0, ccx, ccy, cap.weaponStats.speed||8); const angle = Math.atan2(pred.y-ccy, pred.x-ccx);
        if (cap.weaponStats.hitscan) fireRailgun(cap, cap.weaponStats, angle, true);
        else if (cap.doubleShot) { const bullets = fireDoubleShot(cap, cap.weaponStats, angle, true); bullets.forEach(b=>{b.color="#88ddff";}); playerBullets.push(...bullets); }
        else { const bullets = fireBullets(cap, cap.weaponStats, angle, true); bullets.forEach(b=>{b.color="#88ddff";}); playerBullets.push(...bullets); }
        cap.shootTimer = Math.round((cap.weaponStats.fireInterval || 30) * 1.25);
      }
    }
  }
...
function handleDeployedShipDeath() {
  if (!capitalShipObj || capitalShipObj.hp <= 0) { endGame(false); return; }
  player = capitalShipObj; capitalShipObj._isCapitalAutopilot = false; capitalShipObj = null;
...
function drawCapitalStatusHUD() {
  ...
  ctx.fillStyle = "#aaa"; ctx.font = "9px monospace"; ctx.fillText("HP " + Math.ceil(cap.hp), px+6, py+35);
  const shFrac = Math.max(0, (cap.shields||0)/Math.max(1,cap.maxShields));
  ctx.fillStyle = "#333"; ctx.fillRect(px+6, py+38, panelW-12, 6); ctx.fillStyle = "#0af"; ctx.fillRect(px+6, py+38, (panelW-12)*shFrac, 6);
  ctx.fillStyle = "#aaa"; ctx.font = "9px monospace"; ctx.fillText("SH " + Math.ceil(cap.shields||0), px+6, py+52);
}
...
function drawEnemyCapFormationHUD() {
  ...
  ctx.fillRect(px, py, panelW, panelH); ctx.strokeRect(px, py, panelW, panelH);
  ctx.fillStyle = "#555"; ctx.font = "8px monospace"; ctx.fillText("ENEMY TACTIC", px+5, py+10);
  ctx.fillStyle = color; ctx.font = "bold 10px monospace"; ctx.fillText(label, px+5, py+22);
}
...
function updatePlayer() {
  ...
  if(!_gKeyPrev && keys["KeyG"]) { if (isDeployed) recallToCapital(); else if (isCurrentShipCapital() && deployedShipAvail) deployFromCapital(); }
  _gKeyPrev = !!keys["KeyG"];
  const nemMult=(player.specialActive&&currentShipName==="Nemesis")?2.0:(player.specialActive&&currentShipName==="Starlight")?1.5:(currentShipName==="Vengeance"&&player.revengeActive)?2.0:(currentShipName==="Retribution"&&player.retributionSpeedBuff)?1.5:1.0;
  if(player.boosting){ player.boostTimer--; if(player.boostTimer<=0){player.boosting=false;player.boostCooldown=player.boostCooldownMax;} window.recordBoostFrame?.(); }
  else if(player.boostCooldown>0) player.boostCooldown--;
  else if(keys["ShiftLeft"]||keys["ShiftRight"]){ player.boosting=true;player.boostTimer=player.boostDuration; }
  ...
  if(keys["ArrowUp"]||keys["KeyW"])player.vy-=curAccel; if(keys["ArrowDown"]||keys["KeyS"])player.vy+=curAccel;
  ...
  if(keys["KeyF"]&&missileTimer<=0&&player.missileRack&&player.missileRack.length>0){
    ...
    const _activeKind = player.missileActiveKind; let _fireIdx = -1;
    if (_activeKind) _fireIdx = player.missileRack.findIndex(e=>e.kind===_activeKind);
    if (_fireIdx < 0) _fireIdx = 0;
    const entry = freeAmmo ? (player.missileRack[_fireIdx]||{kind:"standard",tier:player.missileType||2}) : (player.missileRack.splice(_fireIdx, 1)[0]||{kind:"standard",tier:player.missileType||2});
    ...
    if (!freeAmmo && player.missileActiveKind) {
      ...
      if (!_stillHas) { const _kinds = getMissileKindsInRack(); player.missileActiveKind = _kinds[0] || null;
        if (player.missileActiveKind) { const _mk2 = (typeof MISSILE_KINDS!=="undefined"&&MISSILE_KINDS[player.missileActiveKind])||{}; const _kc2 = player.missileActiveKind==="nuke"?"#ff4400":player.missileActiveKind==="emp"?"#44ffcc":player.missileActiveKind==="micro"?"#ffcc44":player.missileActiveKind==="cluster"?"#ff88ff":"#aaddff"; showNotification("Switched to: " + (_mk2.name||player.missileActiveKind), _kc2); }
      }
    }
    const mKind=entry.kind||"standard", mTier=entry.tier||player.missileType||2;
    const mkDef=(typeof MISSILE_KINDS!=="undefined"&&MISSILE_KINDS[mKind])||{aoe:0,lockOn:true,friendly:false,corrosion:false,damageMult:1};
    const mt=MISSILE_TYPES[mTier]||MISSILE_TYPES[2]; const baseDmg=mt.damage*(mkDef.damageMult||1.0);
    let lockTarget=null;
    if(mkDef.lockOn&&enemies.length>0){ const pcx2=player.x+player.w/2,pcy2=player.y+player.h/2; const cos2=Math.cos(player.rotation),sin2=Math.sin(player.rotation); let bestScore=1e9; enemies.forEach(en=>{ const ex=en.x+en.w/2,ey=en.y+en.h/2; const dt=(ex-pcx2)*cos2+(ey-pcy2)*sin2; if(dt<0)return; const perp=Math.abs((ey-pcy2)*cos2-(ex-pcx2)*sin2); if(perp<bestScore){bestScore=perp;lockTarget=en;} }); }
    ...
    _waveMissileFired = true; window.recordMissileFired?.();
  }
  ...
  if (!pdcDisabledThisWave) {
    player.turrets&&player.turrets.forEach(t=>{
      t.shootTimer--; if(t.shootTimer<=0&&t.weaponStats){
        const tbx=player.x+player.w/2,tby=player.y+player.h/2+t.ry; let target=null,bestD=Infinity;
        enemies.forEach(e=>{const d=Math.hypot(e.x+e.w/2-tbx,e.y+e.h/2-tby);if(d<bestD){bestD=d;target=e;}});
        if(target){ const pred=predictPos(target.x+target.w/2,target.y+target.h/2,target.vx||0,target.vy||0,tbx,tby,t.weaponStats.speed||8); const pdcAngle=Math.atan2(pred.y-tby,pred.x-tbx); const pdcOrigin={x:tbx-player.w/2,y:tby-player.h/2,w:player.w,h:player.h};
...
function updateAllies() {
  ...
  allies.forEach((a,i)=>{
    ...
    if(a.isHealer){
      ...
      const fx2=pcx-cos*dist-sin*perp-a.w/2, fy2=pcy-sin*dist+cos*perp-a.h/2;
    ...
    if(allyFormation==="standalone") { fx = a.x; fy = a.y; }
    else if(allyFormation==="defend_capital" && _capitalObj) { const ccx=_capitalObj.x+_capitalObj.w/2, ccy=_capitalObj.y+_capitalObj.h/2; const n=Math.max(1,allies.length); const angle=(Math.PI*2*i/n)+frameCount*0.008; const r=Math.max(_capitalObj.w,_capitalObj.h)*0.7+40+n*8; fx=ccx+Math.cos(angle)*r-a.w/2; fy=ccy+Math.sin(angle)*r-a.h/2; }
    else if(allyFormation==="defend_deployed" && _deployedObj) { const dcx=_deployedObj.x+_deployedObj.w/2, dcy=_deployedObj.y+_deployedObj.h/2; const n=Math.max(1,allies.length); const angle=(Math.PI*2*i/n)+frameCount*0.01; const r=55+n*10; fx=dcx+Math.cos(angle)*r-a.w/2; fy=dcy+Math.sin(angle)*r-a.h/2; }
    else if(pingTarget&&!pingTarget.dead){ const ptcx=pingTarget.x+pingTarget.w/2,ptcy=pingTarget.y+pingTarget.h/2; const n=Math.max(1,allies.length); const angle=(Math.PI*2*i/n); fx=ptcx+Math.cos(angle)*(65+n*12)-a.w/2; fy=ptcy+Math.sin(angle)*(65+n*12)-a.h/2; }
    else if(shieldDown){ const dist=70+i*45,perp=(i-(allies.length-1)/2)*60; fx=pcx+cos*dist-sin*perp-a.w/2; fy=pcy+sin*dist+cos*perp-a.h/2; }
    else if(allyFormation==="front"){ const dist=80+i*50,perp=(i-(allies.length-1)/2)*65; fx=pcx+cos*dist-sin*perp-a.w/2; fy=pcy+sin*dist+cos*perp-a.h/2; }
    ...
    const regenMult=(allyFormation==="behind"&&!pingTarget)?3.0:1.0; regenShieldFaces(a, 0.03*regenMult);
    let closest=null,closestD=1e9;
    ...
    if(_inStandalone && enemies.length > 0) {
      const liveEnemies = enemies.filter(e=>!e.dead); const targetIdx = Math.min(i, liveEnemies.length-1);
      const _standaloneTarget = liveEnemies[targetIdx] || liveEnemies[0];
      if(_standaloneTarget) {
        a._orbitAngle=(a._orbitAngle||0)+0.04; const orbitR=100+_standaloneTarget.w*0.3;
        ...
        a.vx+=(odx/od)*2.5; a.vy+=(ody/od)*2.5; const _as=Math.hypot(a.vx,a.vy);if(_as>20){a.vx*=20/_as;a.vy*=20/_as;}
        a.vx*=0.84;a.vy*=0.84;a.x+=a.vx;a.y+=a.vy; closest = _standaloneTarget;
      }
    }
    const _engageTarget = !_inStandalone && closest && !closest.dead && closestD < 320 && !pingTarget && isSmallEnemy(closest.type) ? closest : null;
    if(_engageTarget) {
      a._orbitAngle = (a._orbitAngle||0) + 0.045; const orbitR = 110;
      ...
      const odx=targetX-a.x, ody=targetY-a.y, od=Math.hypot(odx,ody)||1;
      a.vx += (odx/od)*2.2; a.vy += (ody/od)*2.2; const _as=Math.hypot(a.vx,a.vy); if(_as>18){a.vx*=18/_as;a.vy*=18/_as;}
      a.vx*=0.84; a.vy*=0.84; a.x+=a.vx; a.y+=a.vy;
    }
    ...
    if(a.shootTimer<=0&&closest&&a.weaponStats){
      ...
      if(a.weaponStats.hitscan) fireRailgun(a,a.weaponStats,aimAngle,true);
      else if(a.doubleShot){ const bullets=fireDoubleShot(a,a.weaponStats,aimAngle,true); bullets.forEach(b=>{b.color="#aaffaa";b.damage=(b.damage||0)*fmtMult*vgMult;}); playerBullets.push(...bullets); }
      else { const bullets=fireBullets(a,a.weaponStats,aimAngle,true); bullets.forEach(b=>{b.color="#aaffaa";b.damage=(b.damage||0)*fmtMult*vgMult;}); playerBullets.push(...bullets); }
      const pingMult=(pingTarget&&!pingTarget.dead)?0.9:1.0; a.shootTimer=Math.round((a.weaponStats.fireInterval||28)*pingMult);
    }
  });
}
...
function updateEnemyCapitalAI() {
  ...
  smallEnemies.forEach(s => {
    ...
    capEnemies.forEach(c => { const d = Math.hypot(s.x+s.w/2 - c.x-c.w/2, s.y+s.h/2 - c.y-c.h/2); const tiebreak = capAssign.get(c) * 10; if (d + tiebreak < bestDist) { bestDist = d + tiebreak; bestCap = c; } });
    if (bestCap) { const n = capAssign.get(bestCap); capAssign.set(bestCap, n+1); bestCap._assignedCount = n+1; s._assignedCap = bestCap; s._ecSmallIdx = n; }
    else s._assignedCap = null;
  });
  _enemyCapTimer--;
  if (_enemyCapTimer <= 0) {
    _enemyCapTimer = 180;
    ...
    const weakestAlly = liveAllies.length > 0 ? liveAllies.reduce((w,a) => (!w || a.hp/a.maxHp < w.hp/w.maxHp) ? a : w, null) : null;
    ...
    const isRushing = velDot > 1.5 && spd > 2.0, isPassive = spd < 0.8;
    let isCamping = false;
    if (_playerYHistory.length >= 60) { const mean = _playerYHistory.reduce((s,y)=>s+y,0)/_playerYHistory.length; const variance = _playerYHistory.reduce((s,y)=>s+(y-mean)**2,0)/_playerYHistory.length; isCamping = variance < 900; }
    _enemyCapRegen = 1.0;
  ...
  const _wpTarget = liveAllies.length>0 ? liveAllies.reduce((w,a) => (!w || a.hp/a.maxHp < w.hp/w.maxHp) ? a : w, null) : null;
  smallEnemies.forEach(s => {
    if (!s._assignedCap || s._assignedCap.dead) { s._ecFormation=null; return; }
    const cap = s._assignedCap; const ccx = cap.x+cap.w/2, ccy = cap.y+cap.h/2;
    const toPCx = pcx-ccx, toPCy = pcy-ccy, toPCDist = Math.hypot(toPCx,toPCy)||1;
    const toPCNx = toPCx/toPCDist, toPCNy = toPCy/toPCDist;
    const gi = s._ecSmallIdx || 0, isTopGroup = gi%2===0;
    s._ecFormation = _enemyCapFormation; s._ecFireMult = 1.0;
    switch (_enemyCapFormation) {
      case "vanguard": s._ecTx=ccx+toPCNx*220; s._ecTy=ccy+toPCNy*220; s._ecOrbitR=75; s._ecFireMult=0.80; break;
      case "rearguard": s._ecTx=ccx-toPCNx*200; s._ecTy=ccy-toPCNy*200; s._ecOrbitR=90; break;
      case "surround": s._ecTx=ccx; s._ecTy=ccy; s._ecOrbitR=150+(cap._assignedCount||0)*6; break;
      case "hunt": s._ecTx=capitalShipObj?capitalShipObj.x+capitalShipObj.w/2:pcx; s._ecTy=capitalShipObj?capitalShipObj.y+capitalShipObj.h/2:pcy; s._ecOrbitR=90; s._ecFireMult=0.85; break;
      case "intercept": { const la=allies.filter(a=>!a.dead); const tgt=la.length>0?la[gi%la.length]:null; s._ecTx=tgt?tgt.x+tgt.w/2:pcx; s._ecTy=tgt?tgt.y+tgt.h/2:pcy; s._ecOrbitR=75; break; }
      case "pincer": s._ecTx=pcx; s._ecTy=isTopGroup?Math.max(55,pcy-280):Math.min(GAME_H-55,pcy+280); s._ecOrbitR=50; break;
      case "standalone": { const la=allies.filter(a=>!a.dead); const tgt=la.length>0?la[gi%la.length]:null; s._ecTx=tgt?tgt.x+tgt.w/2:pcx; s._ecTy=tgt?tgt.y+tgt.h/2:pcy; s._ecOrbitR=85; break; }
      case "siege": s._ecTx=pcx; s._ecTy=pcy; s._ecOrbitR=620; break;
      case "wolfpack": s._ecTx=_wpTarget?_wpTarget.x+_wpTarget.w/2:pcx; s._ecTy=_wpTarget?_wpTarget.y+_wpTarget.h/2:pcy; s._ecOrbitR=55; s._ecFireMult=0.75; break;
      default: s._ecTx=pcx; s._ecTy=pcy; s._ecOrbitR=360; break;
    }
  });
  capEnemies.forEach(cap => { if (_enemyCapRegen>1.0) regenShieldFaces(cap, 0.015*(_enemyCapRegen-1.0)); });
}
function updateEnemies() {
  ...
  enemies.forEach(e=>{
    if(e.isShadowComet){ updateShadowCometAI(e); regenShieldFaces(e, 0.01); return; }
    if(e.isShadowVengance){ updateShadowVenganceAI(e); regenShieldFaces(e, 0.008); return; }
    if(e.type==="Dreadnaught"){
      e.x=GAME_W-Math.round(e.w*0.75);e.y+=e.vy;
      if(e.y<=0){e.y=0;e.vy=Math.abs(e.vy);}if(e.y+e.h>=GAME_H){e.y=GAME_H-e.h;e.vy=-Math.abs(e.vy);}
      ...
      e.turrets&&e.turrets.forEach(t=>{
        t.shootTimer--;if(t.shootTimer<=0&&t.weaponStats){
          ...
          let dSide=dAngle-(e.rotation||0);while(dSide>Math.PI)dSide-=Math.PI*2;while(dSide<-Math.PI)dSide+=Math.PI*2;
      ...
      if(e.beamTimer===vkDef.beamWarningFrames){ const baseAngle=Math.atan2(pcy-e.y-e.h/2,pcx-e.x-e.w/2); const count=vkDef.beamCount||1; const spread=vkDef.beamSpread||0; e.beamWarningAngles=[]; for(let b=0;b<count;b++) e.beamWarningAngles.push(baseAngle+(b-(count-1)/2)*spread); }
      if(e.beamTimer<=0){
        const ecx=e.x+e.w/2,ecy=e.y+e.h/2;
        (e.beamWarningAngles||[]).forEach(angle=>{ const cos=Math.cos(angle),sin=Math.sin(angle); const ep=rayEndpoint(ecx,ecy,cos,sin); if(rayHitsRect(ecx,ecy,cos,sin,player))player.hp=-1; allies.forEach(a=>{if(rayHitsRect(ecx,ecy,cos,sin,a))a.hp=-1;}); beamFlashes.push({x1:ecx,y1:ecy,x2:ep.x,y2:ep.y,life:45,maxLife:45,color:"#ff2200",width:14}); });
    ...
    if (e.corrosionTimer>0) { e.corrosionTimer--; e.hp -= (e.corrosionDPS||e.maxHp*0.0001); }
    if (e.staggerTimer>0) e.staggerTimer--;
    if (e.gravityPullTimer>0) { e.gravityPullTimer--; const gx=(e.gravityPullX||0)-(e.x+e.w/2), gy=(e.gravityPullY||0)-(e.y+e.h/2); const gd=Math.hypot(gx,gy)||1; e.vx=(e.vx||0)+gx/gd*1.8; e.vy=(e.vy||0)+gy/gd*1.8; }
    if(e.stunTimer>0) e.stunTimer--;
    else{
      const ecx=e.x+e.w/2,ecy=e.y+e.h/2; const dx=pcx-ecx,dy=pcy-ecy,dist=Math.hypot(dx,dy)||1;
      const ndx=dx/dist,ndy=dy/dist; const accel=getEnemyAccel(e),friction=0.92;
      if(isSmallEnemy(e.type)){
        e.archetypeTimer=(e.archetypeTimer||0)-1;
        if(e.archetype==="adaptive"&&e.archetypeTimer<=0){ e.archetypeMode=e.archetypeMode==="bruiser"?"skirmisher":"bruiser"; e.archetypeTimer=120+Math.floor(Math.random()*180); }
        const mode=e.archetypeMode||e.archetype||"skirmisher";
        let orbitR = mode==="skirmisher"||mode==="suppressor"?(mode==="suppressor"?600:360):mode==="bruiser"||mode==="interceptor"?120:260;
        let tx=pcx,ty=pcy;
        if(mode==="interceptor"&&allies.length>0){ let closestA=allies[0],cad=1e9; allies.forEach(a=>{const d=Math.hypot(a.x+a.w/2-ecx,a.y+a.h/2-ecy);if(d<cad){cad=d;closestA=a;}}); tx=closestA.x+closestA.w/2; ty=closestA.y+closestA.h/2; }
        if (e._ecFormation && e._ecTx !== undefined) { tx=e._ecTx; ty=e._ecTy; orbitR=e._ecOrbitR; }
        ...
        const eSize=ENEMIES[e.type]?.size||2; const isCapital = eSize >= 6;
        const strafeSign=(enemies.indexOf(e)%2===0)?1:-1;
        if(isCapital) {
          const CAPITAL_RANGE = 380 + eSize*10, CAPITAL_FLEE = 200; const strafeMult = eSize>=8 ? 0.35 : 0.55;
          if(dist < CAPITAL_FLEE) { e.vx -= ndx*accel*3; e.vy -= ndy*accel*3; }
          else if(dist > CAPITAL_RANGE + 80) { e.vx += ndx*accel*1.2; e.vy += ndy*accel*1.2; }
          else { e.vx += (-ndy*strafeSign)*accel*strafeMult; e.vy += (ndx*strafeSign)*accel*strafeMult; const re=dist-CAPITAL_RANGE; e.vx += ndx*(re/CAPITAL_RANGE)*accel*0.5; e.vy += ndy*(re/CAPITAL_RANGE)*accel*0.5; }
          enemies.forEach(other=>{ if(other===e||isSmallEnemy(other.type)||other.type==="Dreadnaught")return; const odx=e.x-other.x,ody=e.y-other.y,od=Math.hypot(odx,ody)||1; if(od<220){e.vx+=(odx/od)*accel*1.2;e.vy+=(ody/od)*accel*1.2;} });
          enemies.forEach(other=>{ if(other===e||!isSmallEnemy(other.type)||other.dead)return; const cd=Math.hypot(other.x+other.w/2-ecx,other.y+other.h/2-ecy); if(cd<350) other._capitalBoost=8; });
        } else {
          const TARGET_RANGE=430,FLEE_RANGE=260; const strafeMult=eSize>=4?0.85:1.0;
          if(dist<FLEE_RANGE){e.vx-=ndx*accel*4;e.vy-=ndy*accel*4;}
          else if(dist>TARGET_RANGE+100){e.vx+=ndx*accel*1.5;e.vy+=ndy*accel*1.5;}
          else{ e.vx+=(-ndy*strafeSign)*accel*1.8*strafeMult;e.vy+=(ndx*strafeSign)*accel*1.8*strafeMult; const re=dist-TARGET_RANGE; e.vx+=ndx*(re/TARGET_RANGE)*accel*0.8;e.vy+=ndy*(re/TARGET_RANGE)*accel*0.8; }
          enemies.forEach(other=>{ if(other===e||isSmallEnemy(other.type)||other.type==="Dreadnaught")return; const odx=e.x-other.x,ody=e.y-other.y,od=Math.hypot(odx,ody)||1; if(od<200){e.vx+=(odx/od)*accel;e.vy+=(ody/od)*accel;} });
        }
      }
    ...
    e.turrets&&e.turrets.forEach(t=>{
      t.shootTimer--;if(t.shootTimer<=0&&t.weaponStats){
        ...
        const shipAngle=e.rotation||0; let sideAngle=aimAngle-shipAngle; while(sideAngle>Math.PI)sideAngle-=Math.PI*2;while(sideAngle<-Math.PI)sideAngle+=Math.PI*2;
  ...
  playerBullets.forEach(b=>{
    ...
    enemies.forEach(e=>{
      ...
      if(b.missile&&b.aoeRadius>0){
        b.dead=true; const bx=b.x+b.w/2,by=b.y+b.h/2;
        ...
        else spawnDeathEffect({x:bx-30,y:by-30,w:60,h:60,type:"Raptor"});
        return;
      }
      if(b.piercing){ if(!b.hitSet)b.hitSet=new Set(); if(b.hitSet.has(e))return; b.hitSet.add(e); if(!b.lastEffectFrame||b.lastEffectFrame<frameCount){ spawnHitEffect(b.x+b.w/2,b.y+b.h/2,b); playHitSound(b.category); b.lastEffectFrame=frameCount; } }
      else { spawnHitEffect(b.x+b.w/2,b.y+b.h/2,b); playHitSound(b.category); b.dead=true; }
      applyDamage(e,b);
      if (b._isPlayerBullet) window.recordShotHit?.(b.category, false);
      if (b.chainHops > 0) {
        let hopsLeft = b.chainHops, lastHit = e, dmg = (b.damage || 0) * (b.chainDmgMult || 0.55); const alreadyHit = new Set([e]);
...
function drawEntity(obj) {
  ...
  if (obj.animFrames && obj.animPattern) {
    if (obj._animTick === undefined) obj._animTick = 0;
    obj._animTick++;
    const patIdx = Math.floor(obj._animTick / (obj.animSpeed || 20)) % obj.animPattern.length;
    obj.img = getImage(obj.animFrames[obj.animPattern[patIdx]]);
  }
  ...
  if(hasImg){
    ...
    if(nw>0&&nh>0){ const scale=Math.min(obj.w/nw, obj.h/nh); const dw=nw*scale, dh=nh*scale; ctx.drawImage(obj.img,-dw/2,-dh/2,dw,dh); }
    else ctx.drawImage(obj.img,-obj.w/2,-obj.h/2,obj.w,obj.h);
  }
  else{ctx.fillStyle=obj.color||"#fff";ctx.fillRect(-obj.w/2,-obj.h/2,obj.w,obj.h);}
  const isPlayer=obj===player, isAlly=obj.isAlly, isShadow=obj.isShadowComet, isCapitalAuto=obj._isCapitalAutopilot;
  ...
  if(obj===player&&currentShipName==="Vengeance"&&player.revengeActive){ ctx.save(); ctx.globalAlpha=0.18+0.10*Math.abs(Math.sin(Date.now()/120)); ctx.fillStyle="#ff0044";ctx.shadowColor="#ff0044";ctx.shadowBlur=30; ctx.fillRect(obj.x-4,obj.y-4,obj.w+8,obj.h+8); ctx.restore(); }
  const bx=obj.x,bw=obj.w;
  if(obj.shieldFaces) drawShieldFaces(obj);
  else if(obj.maxShields>0){ ctx.fillStyle="#222";ctx.fillRect(bx,obj.y-17,bw,4); ctx.fillStyle="#0af";ctx.fillRect(bx,obj.y-17,bw*Math.max(0,obj.shields/obj.maxShields),4); }
  ...
  if(obj.archetype&&typeof ARCHETYPE_LABEL!=="undefined"){ const lbl=ARCHETYPE_LABEL[obj.archetype]||""; const col=(typeof ARCHETYPE_COLOR!=="undefined"&&ARCHETYPE_COLOR[obj.archetype])||"#fff"; ctx.save();ctx.font="bold 8px monospace";ctx.fillStyle=col; ctx.fillText(lbl,bx+2,obj.y-19);ctx.restore(); }
}

function drawBullets() {
  [...playerBullets,...enemyBullets].forEach(b=>{
    if(b.vengeanceShot){ ctx.save(); ctx.globalAlpha=0.85; ctx.shadowColor="#ff0000";ctx.shadowBlur=14; ctx.strokeStyle="#ff0000";ctx.lineWidth=2; ctx.beginPath();ctx.arc(b.x+b.w/2,b.y+b.h/2,Math.max(2,b.w*0.3),0,Math.PI*2);ctx.stroke(); ctx.restore(); return; }
    ctx.fillStyle=b.color||"#fff";ctx.fillRect(b.x,b.y,b.w,b.h); if(b.missile){ctx.fillStyle="#ff4400";ctx.fillRect(b.x-6,b.y+1,6,b.h-2);}
  });
}

function drawNukeRings() {
  nukeRings.forEach(r=>{ const alpha=r.life/r.maxLife; ctx.save(); ctx.globalAlpha=alpha*0.85; ctx.strokeStyle=r.color; ctx.lineWidth=Math.max(2, 8*alpha); ctx.shadowColor=r.color; ctx.shadowBlur=20*alpha; ctx.beginPath(); ctx.arc(r.x,r.y,Math.max(1,r.r),0,Math.PI*2); ctx.stroke();
    if(alpha>0.6){ ctx.globalAlpha=(alpha-0.6)*2*0.3; ctx.fillStyle=r.color; ctx.beginPath(); ctx.arc(r.x,r.y,Math.max(1,r.r*0.5),0,Math.PI*2); ctx.fill(); } ctx.restore(); });
}

function drawBeamFlashes() { beamFlashes.forEach(f=>{ const alpha=f.life/f.maxLife; ctx.save();ctx.globalAlpha=alpha;ctx.strokeStyle=f.color||"#88ddff"; ctx.lineWidth=(f.width||4)*(0.5+alpha*0.5);ctx.shadowColor=f.color||"#88ddff";ctx.shadowBlur=24*alpha; ctx.beginPath();ctx.moveTo(f.x1,f.y1);ctx.lineTo(f.x2,f.y2);ctx.stroke();ctx.restore(); }); }
...
function drawBoostHUD() {
  ...
  if(player.boosting){ const frac=player.boostTimer/player.boostDuration; ctx.fillStyle="rgba(0,180,255,0.18)";ctx.fillRect(bx,by,160,28); ctx.fillStyle="rgba(0,200,255,"+(0.5+0.5*Math.abs(Math.sin(Date.now()/80)))+")";ctx.fillRect(bx,by,160*frac,28); ctx.fillStyle="#00eeff";ctx.font="bold 14px monospace";ctx.fillText("BOOST",bx+6,by+19); }
  else if(player.boostCooldown>0){ const frac=1-player.boostCooldown/player.boostCooldownMax; ctx.fillStyle="rgba(80,80,80,0.4)";ctx.fillRect(bx,by,160,28); ctx.fillStyle="rgba(120,120,120,0.7)";ctx.fillRect(bx,by,160*frac,28); ctx.fillStyle="#888";ctx.font="bold 14px monospace";ctx.fillText("BOOST CD "+(player.boostCooldown/60).toFixed(1)+"s",bx+6,by+19); }
  else { ctx.fillStyle="rgba(0,255,120,0.25)";ctx.fillRect(bx,by,160,28); ctx.fillStyle="#00ff88";ctx.font="bold 14px monospace"; ctx.fillText(IS_MOBILE?"BOOST READY":"BOOST READY [SHIFT]",bx+6,by+19); }
  { const dodgeNow=player.boosting?player.dodgeBoosted:player.dodgeBase; ctx.fillStyle="#ffcc00";ctx.font="12px monospace"; ctx.fillText("DODGE "+Math.round(dodgeNow*100)+"%",bx+6,by+44); }
}
...
function render() {
  ...
  if(state==="waveTransition"){
    ...
    ctx.textAlign="center"; ctx.fillStyle="#0af";ctx.font="bold 30px monospace";ctx.fillText(waveTransitionText,GAME_W/2,GAME_H/2-8);
...
// HUD + LOOP
// ============================================================
function getMissileKindsInRack() { const seen = [], rack = player?.missileRack || []; rack.forEach(e => { if (!seen.includes(e.kind)) seen.push(e.kind); }); return seen; }

function showNotification(text, color) {
  let el = document.getElementById("gameNotif");
  if (!el) { el = document.createElement("div"); el.id = "gameNotif"; el.style.cssText = "position:fixed;top:18%;left:50%;transform:translateX(-50%);font:bold 18px monospace;padding:7px 22px;border-radius:8px;pointer-events:none;z-index:8000;transition:opacity 0.4s"; document.body.appendChild(el); }
  el.textContent = text; el.style.color = color || "#fff"; el.style.background = "rgba(0,0,0,0.75)"; el.style.border = "1.5px solid " + (color || "#888"); el.style.opacity = "1";
  clearTimeout(el._t); el._t = setTimeout(() => { el.style.opacity = "0"; }, 1600);
}

function cycleMissileKind() {
  if (!player || !player.missileRack) return; const kinds = getMissileKindsInRack(); if (kinds.length === 0) return;
  const cur = player.missileActiveKind, idx = kinds.indexOf(cur); player.missileActiveKind = kinds[(idx + 1) % kinds.length]; updateHUD();
...
function updateHUD() {
  ...
  { const rack = player.missileRack || []; const mkDefs = typeof MISSILE_KINDS !== "undefined" ? MISSILE_KINDS : {};
...
function isCapitalShip() { return typeof CAPITAL_SHIPS !== "undefined" && CAPITAL_SHIPS.has(currentShipName); }
...
function cycleFormation() {
  ...
  toast.textContent = labels[allyFormation] || allyFormation; toast.style.opacity = "1"; clearTimeout(toast._t); toast._t = setTimeout(() => toast.style.opacity = "0", 1500);
}
